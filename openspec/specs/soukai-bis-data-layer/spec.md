# soukai-bis-data-layer Specification

## Purpose

Kellermeister's RDF/Solid persistence layer is provided by the `soukai-bis` library (replacing `soukai` + `soukai-solid`). Model schemas are declared with `defineSchema` and Zod validators; the migration is behavior-preserving so that data written by the previous version stays readable, and the local-first engine topology (a global IndexedDB engine for ordinary reads/writes, a scoped Solid engine for sync) is retained.
