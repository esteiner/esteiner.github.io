# cellar-bottle-count-header Specification

## Purpose

The cellar view's header names the cellar and, directly beneath that name, states how many bottles the cellar holds in total — the single most useful fact about a cellar, available without scrolling the product list and adding up its row counts. The number describes the cellar itself, so it stays put while the wine-type and search filters narrow the rows below it, and it follows the cellar's contents as bottles are added, moved, or disposed.
