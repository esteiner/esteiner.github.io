...
The profile page SHALL provide, behind each listed cellar name, a delete action. When the cellar contains no bottles, activating the action SHALL delete the cellar and refresh the list. When the cellar still contains bottles, the action SHALL NOT delete it and SHALL instead navigate to that cellar's page so the user can empty it first.

#### Scenario: Delete an empty cellar
- **WHEN** the user activates the delete action for a cellar that contains no bottles
- **THEN** the cellar is deleted
- **AND** the "Keller" list refreshes so the cellar no longer appears

#### Scenario: Delete action on a non-empty cellar navigates instead
...
- **AND** the app navigates to that cellar's page
